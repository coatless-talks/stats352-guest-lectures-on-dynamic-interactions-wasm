...
The `jsonlite` package and 
calls the `fromJSON()` function on the data. The result should be saved into `currency_json`.
...
In the preceeding exercises, we only focused on `USD`. How can improve the analysis by allowing for all currencies rates to be present? e.g.
